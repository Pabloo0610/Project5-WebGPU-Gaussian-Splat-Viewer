...
fn preprocess(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) wgs: vec3<u32>) {
    ...
    // let test_color = vec3<f32>(
    //     clamp(quad_size.x / 0.1, 0.0, 1.0),
    //     clamp(quad_size.y / 0.1, 0.0, 1.0),
    //     0.0
    // );

    let packed_uv = pack2x16float(ndc_pos.xy);
    let packed_size = pack2x16float(quad_size);
    let packed_color_opacity_0 = pack2x16float(vec2<f32>(splat_color.x, splat_color.y));
    let packed_color_opacity_1 = pack2x16float(vec2<f32>(splat_color.z, opacity));
    // let packed_color_opacity_0 = pack2x16float(vec2<f32>(test_color.x, test_color.y));
    // let packed_color_opacity_1 = pack2x16float(vec2<f32>(test_color.z, opacity));

    let packed_conic_radius_0 = pack2x16float(vec2<f32>(conic.x, conic.y));
    let packed_conic_radius_1 = pack2x16float(vec2<f32>(conic.z, radius));

    splats[splat_idx].uv_size[0] = packed_uv;
    splats[splat_idx].uv_size[1] = packed_size;
    splats[splat_idx].color_opacity[0] = packed_color_opacity_0;
    splats[splat_idx].color_opacity[1] = packed_color_opacity_1;
    splats[splat_idx].conic_radius[0] = packed_conic_radius_0;
    splats[splat_idx].conic_radius[1] = packed_conic_radius_1;
